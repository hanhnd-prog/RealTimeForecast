...
for _, station in stations_df.head(-1).iterrows(): # Tam thoi bo qua tram Nam Giang vi chua co so lieu realtime
    ...
    # Load mo hinh da huan luyen de su dung cho du bao
    model_wavelet = load(model_path)
    
    ## 4.4. Dự báo trên tập train và test
    train_pred = model_wavelet.predict(X_train)
    test_pred = model_wavelet.predict(X_test)
    #print('X_test',X_test)
    ...
    ## 4.7. write test_pred into FlowPredictions table of streamflow.db Database
    prediction_made_on_date = df.index[-1].strftime('%Y-%m-%d')
    model_name = "LGBM"
    #print(prediction_made_on_date)
    
    for i in range(n_steps_ahead):
        lead_time = i+1
        
        prediction_for_date = df.index[-1] + timedelta(days=lead_time)
        prediction_for_date = prediction_for_date.strftime('%Y-%m-%d')
        
        predicted_value = test_pred[0][i]
        # print(test_pred[0][i])
        cursor.execute("""
            INSERT INTO FlowPredictions (station_id, prediction_made_on_date, prediction_for_date, lead_time, model_name, predicted_value)
            SELECT ?, ?, ?, ?, ?, ?
            WHERE NOT EXISTS (
                SELECT 1 FROM FlowPredictions
                WHERE station_id = ? AND prediction_made_on_date = ? AND prediction_for_date = ?
            )
        """, (station["station_id"], prediction_made_on_date, prediction_for_date, lead_time, model_name, predicted_value, station["station_id"], prediction_made_on_date,prediction_for_date))
        # cursor.execute("PRAGMA table_info(FlowPredictions)")
